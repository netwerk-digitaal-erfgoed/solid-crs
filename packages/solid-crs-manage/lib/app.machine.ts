      ...
      [AppRootStates.FEATURE]: {
        ...
        states: {
          ...
          [AppFeatureStates.ROUTING]: {
            always: [
              {
                cond: () => history.location.pathname.startsWith('/login'),
                actions: assign({ path: (context, event) => [ '/login', history.location.pathname.replace(/^\/login/, '') ] }),
                target: AppFeatureStates.AUTHENTICATE,
              },
              {
