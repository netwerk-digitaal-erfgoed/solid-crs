...
import type { AccountStore, PodSettings, PodManager, IdentifierGenerator, ResourceIdentifier, OwnershipValidator } from '@solid/community-server';
 ...
 */
export class NoCredentialRegistrationManager {
   ...
   */
  validateInput(input: NodeJS.Dict<unknown>, allowRoot = false): RegistrationParams {

    const {
      webId, podName, template,
    } = input;
   ...
   */
  async register(input: RegistrationParams, allowRoot = false): Promise<RegistrationResponse> {
